...
5. **Registry-driven feature loop**: iterate `FEATURE_REGISTRY` in order, calling `feature.render` for each enabled feature. Registry order (and thus render order): axes → grid → title → legend → tooltip → zoomPan → custom → points → endLabels.
