...
export const endLabelsDef: FeatureDefinition<"endLabels"> = {
  ...
  onZoomRedraw: (ctx, _dims, newX, newY) => {
    ...
    renderEndLabels(ctx.content, ctx.state.currentSeries, newX, newY, ctx.config.xSerie.accessor, ctx.state.endLabelsOptions);
  },
  optionsKey: "endLabelsOptions",
  render: (ctx, _dims) => {
    if (!ctx.flags.hasEndLabels) return;
    if (!ctx.state.endLabelsOptions) return;
    renderEndLabels(ctx.content, ctx.state.currentSeries, ctx.xScale, ctx.yScale, ctx.config.xSerie.accessor, ctx.state.endLabelsOptions);
  },
};
