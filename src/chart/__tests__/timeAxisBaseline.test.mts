...
describe("timeTickFormat", () => {
  it("renders with timeTickFormat string '%Y' producing year-only labels", () => {
    const { selection } = createMockBounds();
    const start = new Date("2020-01-01T00:00:00Z");
    const end = new Date("2024-12-31T23:59:59Z");
    const xScale = scaleTime().domain([start, end]).range([0, 600]);

    // timeTickFormat as a d3 time-format string specifier
    renderXAxis(selection, xScale, 200, {
      tickFormat: (d) => {
        // Simulate timeFormat("%Y") behavior
  ...
  it("renders with timeTickFormat function producing formatted labels", () => {
    const { selection } = createMockBounds();
    const start = new Date("2020-01-01T00:00:00Z");
    const end = new Date("2024-12-31T23:59:59Z");
    const xScale = scaleTime().domain([start, end]).range([0, 600]);

    // timeTickFormat as a custom Date -> string function
    const customFormat = (date: Date) => `${date.getFullYear()}-Q${Math.ceil((date.getMonth() + 1) / 3)}`;
